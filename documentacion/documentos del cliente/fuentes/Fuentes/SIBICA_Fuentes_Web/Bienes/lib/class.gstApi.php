...
class gstApi {
    ...
    public function mostrarPredios($vista, $coordenadas) {
        ...
        $sql = "SELECT oid AS gid, 
                       REPLACE(TO_CHAR(ST_X(ST_CENTROID((ST_DumpPoints(the_geom)).geom)), '99D9999999999'), ',', '.') AS lng,
                       REPLACE(TO_CHAR(ST_Y(ST_CENTROID((ST_DumpPoints(the_geom)).geom)), '99D9999999999'), ',', '.') AS lat,
     ...
     * @param string $cedula      Número de cédula de quien hace el reporte
     * @return array
     */
        ...
        if ($rs->reporte == 0) {
            $sql = "INSERT INTO reporte_predio 
                    (the_geom, 
                     ...
                     foto_reporte, ciudadano_reporte, cedula_reporte, telefono_reporte)
                    VALUES (ST_GeomFromEWKT('SRID=4326;POINT(" . $coordenadas[1] . " " . $coordenadas[0] . ")'),
                            '$predial', '$tipo', NOW(), '$direccion', '$ip',
                            '$fotografia', '$nombre', '$cedula', '$telefono')
            ...
            try {
                ...
                if (!$rsInsert->EOF) {
                    if ($fotografia!="") {
					    $tipoPic = explode('.', $fotografia);
                        $this->reducirIMG($fotografia, $tipoPic[count($tipoPic)-1]);
                    }
                    $num_radicado = $this->wsOrfeo($fotografia, $tipo, $direccion, $predial, $nombre, 
                                                   $correo, $telefono, $coordenadas, $ip, $cedula);

                    return array('success' => 1, 'msg' => '', 'data' => $num_radicado);
                }
            } catch (Exception $ex) {
                return array('success' => 0, 'msg' => BIENES_ORFEO_ERROR);
            }
    ...
    public function mostrarConstrucciones() {
        ...
        $sql = "SELECT const.oid AS gid, 
                       REPLACE(TO_CHAR(ST_X(ST_CENTROID((ST_DumpPoints(const.the_geom)).geom)), '99D9999999999'), ',', '.') AS lng,
                                       --ST_X(ST_CENTROID((ST_DumpPoints(const.the_geom)).geom)) AS lng,
                       REPLACE(TO_CHAR(ST_Y(ST_CENTROID((ST_DumpPoints(const.the_geom)).geom)), '99D9999999999'), ',', '.') AS lat,
    ...
    public function mostrarAmoblamientos($tipo) {
        ...
        foreach ($latlng as $valor) {
            $arr = explode(' ', $valor);
            $LatLng[] = array('lng' => $arr[0], 'lat' => $arr[1]);
        }
    ...
    public function wsOrfeo($imgBase64, $tipo, $direccion, $predial, $nombre, 
        ...
        $destinatario1 = array(
            ...
            'mail' => $correo!=""?$correo:'',
            'otro' => BIENES_ORFEO_CAMPO_OTRO, //$tipo.'|'.$ip.' fecha: ' . date("Y-m-d"),
